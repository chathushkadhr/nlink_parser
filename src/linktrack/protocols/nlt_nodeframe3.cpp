...
NLT_ProtocolNodeFrame3::NLT_ProtocolNodeFrame3(rclcpp::Node *node)
    : NLinkProtocolVLength(
      ...
      publisher_(nullptr),
      node_(node),
      topic("nlink_linktrack_nodeframe3"),
      logger_(rclcpp::get_logger("NLT_ProtocolNodeFrame3")),
      clock_(rclcpp::Clock())
{
  if (node_!=nullptr)
...
void NLT_ProtocolNodeFrame3::HandleData(const uint8_t *serial_data)
{
  ...
  g_nlt_nodeframe3.UnpackData(serial_data, length());

  if (publisher_==nullptr){
    publisher_ = node_->create_publisher<nlink_parser::msg::LinktrackNodeframe3>(topic, 200);
